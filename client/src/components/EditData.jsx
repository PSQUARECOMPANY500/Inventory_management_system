...
import { getEditData, updateEditData, verifyToken } from "../services/api.js";
...
function EditData() {
  const { id } = useParams();
  const [data, setData] = useState([]); // This is the state for the form data
  ...
  const generateAndSaveCode = async () => {
    try {
      ...
      if (response?.data?.message === "Valid Token") {
        await updateEditData(data, { id: id });
      } else {
  ...
  const getDataforEdit = async (id) => {
    let response = await getEditData({ id: id });
      ...
      <Container>
        <FormControl>
          <Typography textAlign="center" varient="h4" component="h1">
            <FormHeader>Update QR Code</FormHeader>
          </Typography>
        </FormControl>
        <FormControl style={{ width: "90%" }}>
          ...
          <CustomDropdown
            name="componentName"
            required
            value={data?.componentName ? data?.componentName : "1"}
            onChange={(e) =>
